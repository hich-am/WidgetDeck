...
export default function CommandPalette() {
  ...
  const { resetAll, addNote, addTask, addHabit, tasks } = useContentStore();
  ...
  const commands: Command[] = useMemo(
    ...
    [expandWidget, resetLayout, resetAll, toggleWidget, enabledWidgets, openFocusMode, openDailyReview, addNote, overdueCount, run]
  );
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "f" || e.key === "F") { e.preventDefault(); const s = useDashboardStore.getState(); s.focusModeOpen ? s.closeFocusMode() : s.openFocusMode(); return; }
  ...
  const sections = useMemo(() => {
    ...
    const source = query ? filtered : filtered;
